...
impl SkinParameters {
    ...
    pub fn apply_reasonable_limits(&mut self) {
        if self.add_skin.len() > 10 {
            warn!("LIMITS: add_skin reducing from {} to 10", self.add_skin.len());
    ...
    fn try_from(params: Vec<(String, String)>) -> Result<SkinParameters, Self::Error> {
        ...
        for (key, value) in params.into_iter() {
            match key.as_str() {
                ...
                "colour_set" => sp.colour_set = Some(value.parse().map_err(|e| json_400(format!("colour_set: {e:?}")))?),
                "background" => sp.background_colour = Some(parse_color(&value).map_err(|e| json_400(format!("background_color: {}", e)))?),
                ...
                "HEAD_SKIN_TOP" | "HEAD_SKIN_BTM" | "MARKINGS" | "ARM_LEFT_SKIN" | "ARM_RIGHT_SKIN" | "LEG_LEFT_SKIN" | "LEG_RIGHT_SKIN" => {
                    sp.slot_colours.insert(key.clone(), parse_color(&value).map_err(|e| json_400(format!("{}: {}", key, e)))?);
                }
                "petpet" => sp.petpet = Some(value.parse().map_err(|e| json_400(format!("petpet: {e:?}")))?),
